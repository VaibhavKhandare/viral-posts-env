...
> An [OpenEnv](https://github.com/meta-pytorch/OpenEnv) environment where an LLM agent manages an Instagram creator account over 7 simulated days, discovering the world through tools rather than being told the rules.
...
async def main():
    ...
    try:
        result = await env.reset(task="weekly_strategic")
...
## Tasks and graders (7 steps each)


| Task                  | Difficulty | Grader focus                                                  |
| --------------------- | ---------- | ------------------------------------------------------------- |
| `weekly_engage`       | Easier     | Total engagement vs theoretical max; burnout penalty          |
| `weekly_strategic`    | Medium     | + tag discovery/exploitation + energy + consistency           |
| `weekly_competitive`  | Hard       | + growth vs competitors + differentiation + content diversity |
...
Auditable rules (all sourced): >5 posts/day → fatigue cliff (Buffer 2.1M); >7 posts/week → weekly cap; ≥4 collabs/week → diminishing returns (Cen 2024); >22h awake → sleep debt (Van Dongen 2003).
...
```bash
git clone https://github.com/VaibhavKhandare/viral-posts-env && cd viraltest
uv sync
