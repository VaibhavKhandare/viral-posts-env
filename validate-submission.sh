...
# ──────────────────────────────────────
log "${BOLD}Step 4/4: Viraltest environment checks${NC} ..."
...
# Test step endpoint with a post action
STEP_CODE=$(curl -s -o "$STEP_OUTPUT" -w "%{http_code}" -X POST \
  -H "Content-Type: application/json" \
  -d '{"action":{"action_type":"post","content_type":"reel","topic":"AI trends","tags":["ai","ml"]}}' \
